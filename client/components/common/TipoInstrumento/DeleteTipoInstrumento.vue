<template>
  <div>
    ...
    <v-dialog v-model="dialog" max-width="500px">
      <v-card>
        <v-card-title class="headline blue darken-4 white--text">
          Eliminar Tipo De Instrumento
        </v-card-title>

        <v-card-subtitle class="mt-5 Subtitle 1">
            <strong><h3>¿Está seguro de que desea eliminar este Tipo De Instrumento.?</h3></strong>
        </v-card-subtitle>
        ...
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" text @click="DeleteTipoInstrumento">
            OK
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <loading :tag="'Borrando Tipo De Instrumento.'"/>
...
export default {
  ...
  methods:{
   ...
   async DeleteTipoInstrumento() {
      const token = Cookies.get('token');
      this.toggleLoading(true);
      try {
        await axios
          .delete(`instrumentoTipo/${this.id}`, {
              headers: { 
