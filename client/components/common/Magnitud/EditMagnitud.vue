   ...
   async editarUsuario(){
      try {
        if(this.$refs.form.validate()){
          await axios.put(`Magnitud/${this.id}`, this.magnitud ,{
              headers: { Authorization: `Bearer ${this.token}` },
            })
            .then(()=>{
              this.alertMsg = "Magnitud actualizado correctamente"
    ...
    async getMagnitud(){
      try {
      await axios.get(`Magnitud/${this.id}`,{
        headers: { Authorization: `Bearer ${this.token}` },
      }).then(resp=>{
        this.magnitud = resp.data.data
      })  
      } catch (error) {
