<template>
  <div class="d-flex justify-end">
    <v-tooltip bottom>
      ...
      <span>Agregar Tipo Calibración</span>
    </v-tooltip>

    <v-dialog v-model="dialog" width="500">
      <v-card>
        <v-card-title class="headline white--text blue darken-4">
          Agregar Tipo Calibración
        </v-card-title>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" text @click="AgregarTipo"> Ok </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
    ...
    async AgregarTipo() {
      try {
        if (this.$refs.form.validate()) {
          await axios
            .post('calibracionTipo', this.tipo, {
              headers: { Authorization: `Bearer ${this.token}` },
            })
            .then(() => {
              this.alertMsg = 'Calibracion agregada correctamente'
